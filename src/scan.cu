...
__global__ void scan_d_kernel(
    ...
    __shared__ data_t Y_s[warpSize * warpSize];
    __shared__ data_t sdata[warpSize];
    ...
    do {
        ...
        if (threadIdx.x == 0) {
            bid_s = atomicInc(block_counter, 1);
        }
        ...
        for (int i = threadIdx.x; i < warpSize * warpSize; i += blockDim.x) {
            data_t yi = (i < len) ? X[base + i] : 0;

            yi += __shfl_up_sync(__activemask(), yi, 1);
            yi += __shfl_up_sync(__activemask(), yi, 2);
            yi += __shfl_up_sync(__activemask(), yi, 4);
            yi += __shfl_up_sync(__activemask(), yi, 8);
            yi += __shfl_up_sync(__activemask(), yi, 16);
...
__host__ void malloc_persisting_data(T ** ptr_addr, unsigned int n, float hit_ratio = 1.0f) {
    ...
    stream_attribute.accessPolicyWindow.base_ptr  = reinterpret_cast<void*>(inclusive_prefix_descriptor_table_d);
    stream_attribute.accessPolicyWindow.num_bytes = inclusive_prefix_descriptor_table_size;
...
__host__ void test_scan_handcraft(const data_t * X_h, const data_t * ans_h, int N) {
    ...
    int partition_size = warpSize * warpSize;
...
__host__ int main(int argc, char ** argv) {
    ...
    utils::random_fill_h<data_t>(X_h, N, 0, 100); 
